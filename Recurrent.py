...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001,
                 frame_height=84, frame_width=84, agent_history_length=1):
        """
        Args:
        ...
        self.LSTM = tf.keras.layers.LSTM(units=512, input_shape=(1024,), dtype=tf.dtypes.float32)(tf.expand_dims(tf.keras.layers.Flatten()(self.conv4), 0))
        # Splitting into value and advantage stream
        self.valuestream, self.advantagestream = tf.keras.layers.Lambda(lambda tensor: tf.split(tensor, 2, 1))(self.LSTM)
...
class ReplayMemory(object):
    ...
    def __init__(self, size=1000000, frame_height=84, frame_width=84,
        ...
        self.frames = np.empty((self.size, self.frame_height, self.frame_width, self.agent_history_length),
                               dtype=np.uint8)
        self.terminal_flags = np.empty(self.size, dtype=np.bool)

        # Pre-allocate memory for the states and new_states in a minibatch
        self.states = np.empty((self.batch_size * self.sequence_len, self.frame_height, self.frame_width, self.agent_history_length),
            dtype=np.uint8)
    ...
    def _get_state(self, index):
        ...
        return self.frames[index:index + self.sequence_len, ...]
        ...
        for i in range(self.batch_size):
            while True:
                ...
                if index >= self.current and index - self.agent_history_length <= self.current:
                    continue
                # Check that we are not pulling out any memory that span over 2 episodes
                if self.terminal_flags[index:index + self.sequence_len].any():
                    continue
                break
    ...
    def get_minibatch(self):
        ...
        for i, idx in enumerate(self.indices):
            self.states[i * (self.sequence_len):(i + 1) * (self.sequence_len)] = self._get_state(
                idx)  # State contains 4 consecutive frames
            a.extend(self.actions[idx:idx + self.sequence_len])
            r.extend(self.rewards[idx:idx + self.sequence_len])
            t.extend(self.terminal_flags[idx:idx + self.sequence_len])

        return self.states[:(len(self.indices) + 1) * self.sequence_len], np.array(a), np.array(r), np.array(t)
...
def learn(replay_memory, main_dqn, target_dqn, batch_size, gamma):
    ...
    All_frames, All_actions, All_rewards, All_terminal_flags = replay_memory.get_minibatch()
    # print(len(All_frames), len(All_actions), len(All_rewards), len(All_terminal_flags))
    # Split into microbatches of consequtive frames
    losses = []
    errors = []
    for cP in range(2, len(All_frames) - 1, 4):
        states, actions, rewards, new_states, terminal_flags = All_frames [cP - 2:cP + 1], \
                                                               All_actions[cP - 2:cP + 1], \
                                                               All_rewards[cP - 2:cP + 1], \
                                                               All_frames [cP - 1:cP + 2], \
                                                               All_terminal_flags[cP - 2:cP + 1]
        ...
        losses.append(main_dqn.model.fit(states, wanted_Qs, verbose=0).history['loss'])
        ...
        errors.append(target_q - expected_q)
...
class Atari(object):
    ...
    def step(self, action):
        ...
        self.state = processed_new_frame
