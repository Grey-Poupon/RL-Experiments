...
class NoisyDense(tf.keras.layers.Dense):
    ...
    def build(self, input_shape):
        ...
        #(trainable weights) Second kernel for controlling randomness
        ...
        self.input_spec = tf.keras.layers.InputSpec(min_ndim=2, axes={-1: self.input_dim})
        self.built = True

    def call(self, inputs):
        # Generation of the matrix with random numbers (newly generated with every call) - vector version
        # (see Noisy Nets Paper) would be more efficient.
        ...
        output = tf.keras.backend.dot(inputs, w)

        if self.use_bias:
            # Generation random vector for bias random
...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001,
        ...
        # Replace Dense Layers With NoisyDense
        self.advantage = NoisyDense(units=self.n_actions, activation=tf.nn.relu,
                                    kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                    bias_initializer=tf.keras.initializers.VarianceScaling(scale=2))(self.advantagestream)

        self.value = NoisyDense(units=self.n_actions, activation=tf.nn.relu,
                                kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                bias_initializer=tf.keras.initializers.VarianceScaling(scale=2))(self.valuestream)
...
class ExplorationExploitationScheduler(object):
    ...
    def __init__(self, DQN, n_actions,  replay_memory_start_size=50000, max_frames=25000000):
        """
        Args:
            DQN: A DQN object
            n_actions: Integer, number of possible actions
    ...
    while frame_number < MAX_FRAMES:  #
        ...
        while epoch_frame < EVAL_FREQUENCY:
            ...
            for _ in range(MAX_EPISODE_LENGTH):
                ...
                if terminal:
                    #print("Run: " + str(run) + "  Reward: " + str(episode_reward_sum) + "  Frame Count: " + str(frame_number))
                    terminal = False
                    break
