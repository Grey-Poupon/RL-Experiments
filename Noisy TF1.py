...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001,
        ...
        self.noisy_value     = self.noisy_dense(self.valuestream,     name='noisy_value',     size=1,              activation_fn=tf.nn.relu)
        self.noisy_advantage = self.noisy_dense(self.advantagestream, name='noisy_advantage', size=self.n_actions, activation_fn=tf.nn.relu)
