...
class DQN(object):
    ...
    def __init__(self, n_actions, hidden=1024, learning_rate=0.00001,
        ...
        self.conv1 = tf.layers.conv2d(
            inputs=self.inputscaled, filters=16, kernel_size=[8, 8], strides=4,
            kernel_initializer=tf.variance_scaling_initializer(scale=2),
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv1')
        self.conv2 = tf.layers.conv2d(
            inputs=self.conv1, filters=32, kernel_size=[4, 4], strides=2,
            kernel_initializer=tf.variance_scaling_initializer(scale=2),
            padding="valid", activation=tf.nn.relu, use_bias=False, name='conv2')

        self.flatten = tf.layers.dense(
            inputs=tf.layers.flatten(self.conv2), units=256,
            kernel_initializer=tf.variance_scaling_initializer(scale=2), name='flat')


        # Splitting into value and advantage stream
        self.valuestream, self.advantagestream = tf.split(self.flatten, [128,128], 1)
...
def train():
    ...
    with tf.Session() as sess:
        sess.run(init)
        #saver.restore(sess, "/home/Kapok/Saves/Breakout/Weights/9866587")
        ...
        while frame_number < MAX_FRAMES:
            ...
            saver.save(sess, "/home/Kapok/Saves/Base/Pong/Weights/"+str(frame_number))
            np.savez(        "/home/Kapok/Saves/Base/Pong/Saves/Memory", my_replay_memory.actions, my_replay_memory.rewards, my_replay_memory.frames,my_replay_memory.terminal_flags)
            np.save(         "/home/Kapok/Saves/Base/Pong/Logs/Logs_"+str(frame_number), log_list)
            logs = []
            print("saved")
